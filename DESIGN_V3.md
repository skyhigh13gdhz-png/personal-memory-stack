...
1. **证据**：原始记录与可引用的事实/事件；
2. **派生视图**：按时间窗口或 Memory Subject 组织的可重建读模型；
...
长期记忆（Memory Subject Views）
└─ 人物 / 项目 / 长期主题 / 资产与策略 / 系统与工具
```
...
底层属于 Temporal Projection，产品与 Obsidian 统一使用“周期回顾”。
...
`00/10/20/90` 的系统区块可重建、默认只读；`30-人工知识` 由用户所有，系统不覆盖。需要在长期记忆内人工编辑时，使用独立 human-owned block/file。
...
- “没有记录”不等于“没有发生”；
- 空栏目隐藏，概览与分类不重复；
- 只有 material change 才触发长期记忆状态评估。
- 正文采用“栏目 → 具体项目/子主题 → 带语义标签的条目”的总分结构；
- 默认人类视图隐藏机器标识和证据索引，审计信息通过 HTML 注释或独立审计视图保留；
- 保留用户稳定措辞和领域术语，不把私人记录统一改写为公文语气；
- 事实、程序计算与模型推断分层，推断不得冒充原始事实。
...
建立 7–14 天 golden set，人工标注关键事实、禁止推断项、隐私项和真正有价值的 Subject 状态变化。
...
1. 周期回顾与长期记忆的 Obsidian 命名已确定，后续不再用目录解决数据模型问题；
