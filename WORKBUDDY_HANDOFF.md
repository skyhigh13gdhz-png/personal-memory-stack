...
同时把原文、baseline、未来 Event-first 候选随机化为 A/B/C 对照包。测试只用合成数据和 fake provider；
不得读取真实 Vault、调用外部 API、写正式日报或修改 freeze marker。
...
建立纯合成反例：一个事件多主题、同主题多事件、无标题长文、跨文档补记、已勾选但无结果的待办、
...
用户决定的产品语义，立即停在安全 checkpoint；除此之外不要因等待复核而停工。
