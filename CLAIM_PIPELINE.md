...
模型返回为空的 evidence 也视为候选失败。根对象或 claims 数组本身非法时整批隔离为 `*.rejected.json`；单条 Claim 非法时仅进入包内 `rejected_candidates`。两者都保持 `600` 权限，被拒绝内容不得投影。剩余候选仍必须通过整包 95% 召回门禁。

证据对齐先要求完全逐字命中。只有中英文引号等明确标点等价、且归一化后在原文唯一命中时，才可以用真实原文片段替换模型引文，并记入 `evidence_repairs`。缺字、多字、改词或多处命中均继续拒绝。

Prompt v3 同时为“日回顾”与“持续脉络”抽取，要求在输出前内部检查饮食、睡眠、运动、项目、财务、家庭关系和宠物档案等覆盖；覆盖要求不降低逐字证据门槛。
